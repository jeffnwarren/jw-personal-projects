...
//
// NEW IN v11.10.1:
// 11a. Page indicator ordinal accuracy (enhancement to existing check #4):
//     The indicator previously only checked that "Page N" appeared in the text.
//     The display format is "Page N (X/Y)" where X is the 1-based ordinal position of section
//     N in the sorted section list, and Y is the total number of sections. Both X and Y are
//     now verified independently, catching bugs where:
//       - totalSections is wrong (e.g. counting non-game sections or hidden sections)
//       - The ordinal position is off (sort order mismatch between the book JS and the DOM)
//       - The (X/Y) part is missing entirely (indicator only showing "Page N")
//     The expected ordinal is computed from the same sectionsOrdered array the script builds
//     at startup, which mirrors the book's own sortedSections array exactly.
//     Mismatches are reported as INDICATOR failures with the full expected vs. actual text.
//
// NEW IN v11.11.0:
// 12. JavaScript console error detection:
//     Attaches page.on('pageerror') and page.on('console') listeners at startup before any
//     navigation. Collects all uncaught JS exceptions and console.error() calls throughout the
//     entire run (BFS, extra tests, everything). Reports each as a JS_ERROR failure. This catches
//     errors that don't manifest as visible navigation failures but can cause subtle state corruption.
//
// 13. Link text vs. target mismatch (static check S6):
//     For each .page-link, reads the surrounding prose text for "page N" / "Page N" mentions and
//     compares the mentioned number to the link's actual onclick/data-target target. Catches the
//     most common undetected OCR error: prose says "Turn to page 14" but the link fires
//     goToSection(15), and both sections exist so no structural test catches it.
//
// 14. Section number gap report (static check S8):
//     After model build, computes the range [1..maxSectionNumber] and reports which numbers are
//     absent. Helps distinguish intentional omissions (section 7 in Dungeon of Dread) from
//     accidental deletions. Reported as a NOTE.
//
// 15. Missing .page-number div detection (static check S7):
//     Each game section should contain a .page-number div for reader orientation. Sections
//     missing it are reported as MISSING_PAGE_NUM notes.
//
...
function makeLogger(outputLogOpt, bookFilePath) {
  ...
  // NOTE: Do NOT add file-appending logic here — orig.log/error/warn internally call
  // process.stdout/stderr.write, which are already hooked below to append to the log file.
  // Adding appendLine here would double-write every line.
  console.log = (...args) => { orig.log(...args); };
  console.error = (...args) => { orig.error(...args); };
  console.warn = (...args) => { orig.warn(...args); };
...
(async () => {
  ...
  if (!filePath) {
    console.error('Usage: node qa-gamebook-v11.9.js [--headed] [--progress] [--skipExtras] [--minContent=40] [--progressEvery=10] [--maxStates=5000] [--slowMo=0] [--timeoutMs=1500] [--outputLog[=<n>]] "C:\\path\\to\\Gamebook.html"');
    process.exit(1);
  }
  ...
  const startStamp = new Date(runStartTime).toLocaleString();
  logger.writeLine(`QA run started: ${startStamp}`);
  ...
  // NEW v11.11.0: Collect uncaught JS exceptions and console.error calls throughout the run.
  // Listeners are registered before any navigation so nothing is missed.
  const jsErrors = [];
  page.on("pageerror", (err) => {
    jsErrors.push({ kind: "uncaught", msg: err.message || String(err) });
  });
  page.on("console", (msg) => {
    if (msg.type() === "error") {
      jsErrors.push({ kind: "console.error", msg: msg.text() });
    }
  });
  ...
  const model = await page.evaluate((minContent) => {
    ...
    // NEW v11.11.0: Link text vs. target mismatch detection.
    // For each .page-link, read the surrounding text for a mentioned page number and compare
    // to the link's actual onclick/data-target. Catches the most common undetected OCR error:
    // prose says "Turn to page 14" but the link fires goToSection(15).
    const linkTextMismatches = [];
    for (const s of sections) {
      const links = Array.from(s.querySelectorAll(".page-link"));
      for (const link of links) {
        // Determine actual target number from onclick or data-target
        let actualTarget = null;
        const dt = link.getAttribute("data-target");
        if (dt) {
          const dtm = dt.match(/(\d+)/);
          if (dtm) actualTarget = parseInt(dtm[1], 10);
        } else {
          const oc = link.getAttribute("onclick") || "";
          const ocm = oc.match(/goToSection\((\d+)\)/);
          if (ocm) actualTarget = parseInt(ocm[1], 10);
        }
        if (actualTarget === null) continue;

        // Read the prose text that surrounds or contains this link.
        // Check the link's own text, its parent .choice-text, or closest container.
        const textSources = [
          link.textContent || "",
          link.closest(".choice-text") ? link.closest(".choice-text").textContent : "",
          link.parentElement ? link.parentElement.textContent : "",
        ];
        const combinedText = textSources.join(" ");

        // Extract all "page N" / "Page N" mentions from the surrounding text
        const pageRefs = [...combinedText.matchAll(/(?:page|pg\.?)\s+(\d+)/gi)];
        for (const ref of pageRefs) {
          const mentionedNum = parseInt(ref[1], 10);
          if (mentionedNum !== actualTarget) {
            linkTextMismatches.push({
              sectionId: s.id,
              mentionedNum,
              actualTarget,
              text: combinedText.replace(/\s+/g, " ").trim().slice(0, 80),
            });
          }
        }
      }
    ...
    // NEW v11.11.0: Missing .page-number div detection.
    // Each game section should contain a .page-number div for reader orientation.
    const missingPageNumber = [];
    for (const s of sections) {
      if (!s.querySelector(".page-number")) {
        missingPageNumber.push(s.id);
      }
    }
  ...
  // Static integrity checks
  {
    ...
    // NEW v11.11.0: Link text vs. target mismatch (S6)
    // The prose says "Turn to page X" but the link fires goToSection(Y).
    for (const m of model.linkTextMismatches) {
      failures.push({
        type: "LINK_TEXT",
        where: m.sectionId,
        msg: `Prose mentions page ${m.mentionedNum} but link target is section-${m.actualTarget} — "${m.text}"`,
      });
    }

    // NEW v11.11.0: Missing .page-number div (S7)
    for (const id of model.missingPageNumber) {
      notes.push({
        type: "MISSING_PAGE_NUM",
        where: id,
        msg: "Section is missing a .page-number div — no visible page number for the reader",
      });
    }

    // NEW v11.11.0: Section number gap report (S8)
    // Reports which section numbers are absent between 1 and the max section number.
    {
      const nums = model.sectionNums.map(x => x.n);
      ...
      if (gaps.length) {
        notes.push({
          type: "SECTION_GAPS",
          where: "STRUCTURE",
          msg: `Missing section numbers in range 1–${maxNum}: ${gaps.join(", ")} (${gaps.length} gap${gaps.length === 1 ? "" : "s"})`,
        });
      }
    }
  }
  ...
  while (queue.length && states < maxStates) {
    ...
    if (opts.progress && states % opts.progressEvery === 0) logger.writeRaw(".");

    await gotoSectionByPath(state.path);

    // -----------------------------------------
    // Page indicator check (once per unique section ID)
    // Verifies both the section number AND the (ordinal/total) part of the indicator text.
    // Expected format: "Page N (X/Y)" where X = 1-based ordinal in sorted section list, Y = total.
    // The (X/Y) check was added in v11.10.1 after a real phone showed "Page 13 (12/126)" —
    // correct behaviour for a book with non-contiguous section numbers (section 7 was missing),
    // but the previous check would not have caught a bug in the ordinal or total calculation.
    if (!checkedIndicator.has(state.at)) {
      checkedIndicator.add(state.at);
      const expectedN = numFromSectionId(state.at);
      if (expectedN != null) {
        // Compute expected ordinal from our own sectionsOrdered (mirrors book's sortedSections)
        const expectedOrdinal = sectionsOrdered.indexOf(state.at) + 1; // 1-based
        const expectedTotal = sectionsOrdered.length;

        const indicatorResult = await page.evaluate(({ n, ordinal, total }) => {
          const el = document.getElementById("page-indicator");
          if (!el) return { found: false };
          const text = el.textContent.trim();

          // Check section number
          const hasPageN = text.includes("Page " + n + " ") || text === "Page " + n;

          // Parse the (X/Y) ordinal part
          const ordinalMatch = text.match(/\(\s*(\d+)\s*\/\s*(\d+)\s*\)/);
          const parsedOrdinal = ordinalMatch ? parseInt(ordinalMatch[1], 10) : null;
          const parsedTotal   = ordinalMatch ? parseInt(ordinalMatch[2], 10) : null;

          const ordinalOk = parsedOrdinal === ordinal;
          const totalOk   = parsedTotal   === total;

          return { found: true, text, hasPageN, parsedOrdinal, parsedTotal, ordinalOk, totalOk };
        }, { n: expectedN, ordinal: expectedOrdinal, total: expectedTotal });

        if (indicatorResult.found) {
          // Section number wrong
          if (!indicatorResult.hasPageN) {
            failures.push({
              type: "INDICATOR",
              where: state.at,
          ...
          // Ordinal (X/Y) present but wrong
          if (indicatorResult.parsedOrdinal !== null && !indicatorResult.ordinalOk) {
            failures.push({
              type: "INDICATOR",
              where: state.at,
              msg: `#page-indicator ordinal wrong: shows (${indicatorResult.parsedOrdinal}/${indicatorResult.parsedTotal}), expected (${expectedOrdinal}/${expectedTotal}) — full text: "${indicatorResult.text}"`,
            });
          }
          if (indicatorResult.parsedTotal !== null && !indicatorResult.totalOk) {
            failures.push({
              type: "INDICATOR",
              where: state.at,
              msg: `#page-indicator total wrong: shows (${indicatorResult.parsedOrdinal}/${indicatorResult.parsedTotal}), expected (${expectedOrdinal}/${expectedTotal}) — full text: "${indicatorResult.text}"`,
            });
          }

          // (X/Y) part missing entirely (only warn as a note — some books may omit it)
          if (indicatorResult.parsedOrdinal === null && indicatorResult.hasPageN) {
            notes.push({
              type: "NOTE",
              where: state.at,
              msg: `#page-indicator missing (ordinal/total) part: "${indicatorResult.text}" — expected "Page ${expectedN} (${expectedOrdinal}/${expectedTotal})"`,
            });
          }
        }
        // If element not found at all, skip silently — book may not use this element in all states
      }
    }
  ...
  // -----------------------------------------
  // NEW v11.11.0: Report any JS errors collected during the entire run
  if (jsErrors.length) {
    ...
    notes.push({ type: "NOTE", where: "JS_ERRORS", msg: "No uncaught JS exceptions or console.error calls during the entire run." });
  }
  ...
  const endStamp = new Date(runEndTime).toLocaleString();
  ...
  logger.writeLine(`QA run ended: ${endStamp}  |  Duration: ${durationMin}m ${durationSec}s`);
