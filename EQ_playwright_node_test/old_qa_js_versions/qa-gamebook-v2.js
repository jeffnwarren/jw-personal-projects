...
(async () => {
  ...
  async function gotoSectionId(sectionId) {
    ...
    const fn = (typeof window.goToSection === 'function')
      ? window.goToSection
      : (typeof goToSection === 'function' ? goToSection : null);
    if (!fn) throw new Error('No goToSection function found (neither window.goToSection nor global goToSection).');
    return fn(num, false);
  }, n);
    // allow UI updates
