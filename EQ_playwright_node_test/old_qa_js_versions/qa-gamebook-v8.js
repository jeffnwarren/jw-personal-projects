...
(async () => {
  ...
  async function gotoSectionByPath(pathIds) {
    ...
    await page.goto(url, { waitUntil: "load" });
    // Keep QA runs deterministic: don’t resume prior saved progress.
    await page.evaluate(() => {
      try { localStorage.clear(); } catch {}
      try { sessionStorage.clear(); } catch {}
      document.body.classList.add("qa-mode");
      // Some builds build sortedSections lazily; ensure indexes are ready.
      try { if (typeof refreshSectionIndex === "function") refreshSectionIndex(); } catch {}
    });
    ...
    for (let i = 0; i < pathIds.length - 1; i++) {
      ...
      if (!link) {
        ...
        if (fromN != null && toN != null && Math.abs(toN - fromN) === 1) {
          ...
          // Prefer calling the engine directly (more reliable than clicking an overlay element).
          ...
          // If the engine call didn’t exist for some reason, fall back to dispatching a click.
