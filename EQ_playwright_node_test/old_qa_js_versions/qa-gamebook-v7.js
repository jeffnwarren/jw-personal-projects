...
(async () => {
  ...
  async function gotoSectionByPath(pathIds) {
    ...
    await page.goto(url, { waitUntil: "load" });
    // Enable QA mode visuals (page flip arrows, breadcrumb) if the build supports it.
    ...
    if (!cur) {
      await page.evaluate((id) => {
        const n = (String(id).match(/section-(\d+)/) || [])[1];
        if (n && typeof goToSection === "function") {
          goToSection(parseInt(n, 10), false);
          return;
        }
        // last resort: set the active class only
      ...
      notes.push({ type: "NOTE", where: firstTarget, msg: "Fallback: initialized first section (no active section after load/begin)." });
    ...
    if (cur !== firstTarget) {
      ...
      if (cur !== firstTarget) {
        ...
        notes.push({ type: "NOTE", where: firstTarget, msg: `Fallback: initialized active section to ${firstTarget} (couldn't reach via UI).` });
        await page.waitForTimeout(40);
      }
    }
    ...
    for (let i = 0; i < pathIds.length - 1; i++) {
      ...
      if (!link) {
        // If there is no explicit choice link, attempt to use the page-flip arrows (linear navigation).
        const fromN = numFromSectionId(fromId);
        const toN = numFromSectionId(toId);

        if (fromN != null && toN != null && Math.abs(toN - fromN) === 1) {
          const arrowSel = (toN > fromN)
            ? `section#${fromId} .page-nav-right`
            : `section#${fromId} .page-nav-left`;

          // Try a robust click (some builds position arrows over content / need force)
          const arrow = await page.$(arrowSel);
          if (!arrow) throw new Error(`Could not find page-nav arrow (${arrowSel}) from ${fromId} to ${toId}`);

          try {
            await page.locator(arrowSel).click({ force: true, timeout: 500 });
          } catch {
            // Fallback: invoke the handler directly if it exists (inline onclick usually requires global functions)
            ...
            // As a last fallback, dispatch a click event
            try {
          ...
          // As a fallback, try any matching choice link anywhere (in case markup is slightly different)
      ...
      // Wait until the engine activates the expected section.
      try {
        ...
        throw new Error(`After clicking ${fromId} -> ${toId}, active section is ${now ?? "null"}`);
      }
    }
  }
  ...
  while (queue.length && states < maxStates) {
    ...
    // Primary navigation: explicit choice links (.page-link).
    // Fallback navigation: if a section has no .page-link choices, assume the page-flip arrow goes to the next numeric section (section-(n+1)).
    let targets = outDegree.get(state.at) || [];

    if (!targets.length) {
      const m = String(state.at).match(/section-(\d+)/);
      if (m) {
