...
// Copy the gamebook html (e.g. Dungeon_of_Dread_Final_2026-02-20_0727_CST.html) and latest qa_gamebook-<version>.js file (e.g. qa-gamebook-v11.10.0.js) to the project folder.
//
// PURPOSE:
...
//  - [Mobile] Viewport meta tag audit (width=device-width, no user-scalable=no)
//  - [Mobile] Horizontal overflow at 390px portrait and 844px landscape
//  - [Mobile] Touch target size audit on interactive elements (44px minimum)
//  - [Mobile] Swipe navigation (left/right gestures trigger correct page-flip)
//  - [Mobile] Landscape layout (nav arrows in bounds, no overflow)
//  - [Mobile] iOS input zoom prevention (font-size >= 16px on #page-jump)
//
// WHAT IS BFS?
...
//
// NEW IN v11.10 — MOBILE TEST SUITE:
// A dedicated set of mobile-browser checks that run after the extras block.
// The book already uses a 390x844 Playwright viewport, so portrait-layout checks are
// essentially "free" extensions of what the browser already sees. Landscape and swipe
// tests adjust the viewport and dispatch touch events, then restore.
// Skip the entire suite with --skipMobile.
//
// 12. Viewport meta tag audit: confirms <meta name="viewport"> is present and well-formed.
//     Checks for width=device-width, initial-scale=1.0 (required for correct mobile scaling),
//     and flags user-scalable=no or maximum-scale=1 (these block accessibility pinch-zoom,
//     violating WCAG 1.4.4 and causing App Store review rejections on iOS).
//
// 13. Horizontal overflow — portrait (390px): samples the first, middle, and last sections
//     and checks document.documentElement.scrollWidth against window.innerWidth. Any section
//     wider than the viewport causes horizontal scrolling on real phones, which breaks the
//     reading experience and hides content. Flagged as MOBILE_OVERFLOW failures.
//
// 14. Touch target size audit: navigates to a section with choices and a section without,
//     then measures the hit area (via getBoundingClientRect) of every interactive element
//     class in that section: .page-link, .control-btn, .bookmark-btn, .page-nav.
//     Anything with height < 44px OR width < 44px is flagged as a TOUCH_TARGET note.
//     (44px = Apple HIG minimum; 48dp = Material Design minimum. We use 44 as the floor.)
//     Inline .page-link anchors are almost always too small and are the primary finding here.
//
// 15. Swipe navigation: the book uses global vars touchStartX / touchEndX and a plain-scope
//     handleSwipe() function. Because these are genuine globals (not inside an IIFE), the
//     test can set them directly and call handleSwipe() inside page.evaluate(), then verify
//     the active section changed. Tests: right-to-left (→ goToNextPage), left-to-right
//     (→ goToPrevPage), and a sub-threshold swipe (< 50px, should not navigate).
//     Also dispatches real TouchEvents on document to confirm the event listeners fire.
//
// 16. Landscape layout (844x390): switches Playwright's viewport to landscape, navigates
//     to the first game section, and checks: (a) no horizontal overflow, (b) both
//     .page-nav-left and .page-nav-right are fully within the viewport bounds. The book's
//     @media rule repositions the arrows to 5px from each edge below 560px width, so at
//     844px they revert to calc(50% - 270px) = 152px from each edge — still well within
//     the 844px-wide viewport. This test catches regressions in that CSS rule.
//     Viewport is restored to 390x844 at the end.
//
// 17. iOS input zoom prevention: iOS Safari auto-zooms the entire page when the user taps
//     an <input> whose computed font-size is below 16px. This is jarring and disorienting
//     in a book UX. Checks the computed font-size on #page-jump (the "Go to page" field)
//     and flags anything below 16px as a MOBILE_INPUT_ZOOM note.
//
...
//
//   --skipMobile
//       Skips the mobile browser test suite (tests 12-17).
//       Use this if you only need functional/BFS coverage and not mobile-layout validation.
//
//   --skipExtras
...
//
// USAGE:
//   node qa-gamebook-v11.10.js [flags] "C:\path\to\YourGamebook.html"
//
// Example:
//   node qa-gamebook-v11.10.js --headed --progress --slowMo=50 --outputLog "C:\path\to\YourGamebook.html"
//   node qa-gamebook-v11.10.js --skipExtras --skipMobile --progress "C:\path\to\YourGamebook.html"
//
// NOTES:
...
function parseArgs(argv) {
  ...
  for (const a of argv) {
    ...
    if (a === "--skipMobile") { opts.skipMobile = true; continue; }
    const m = a.match(/^--([^=]+)=(.*)$/);
    if (m) {
      const k = m[1];
      const v = m[2];
...
(async () => {
  ...
  if (!filePath) {
    console.error('Usage: node qa-gamebook-v11.10.js [--headed] [--progress] [--skipExtras] [--skipMobile] [--minContent=40] [--progressEvery=25] [--maxStates=5000] [--slowMo=0] [--timeoutMs=1500] [--outputLog[=<n>]] "C:\\path\\to\\Gamebook.html"');
    process.exit(1);
  }
  ...
  // -----------------------------------------
  // MOBILE TEST SUITE (tests 12-17)
  // Runs after the extras block. Skip with --skipMobile.
  // Uses the existing page (390x844 viewport) for portrait tests, adjusts viewport for
  // landscape, and creates a separate hasTouch context for the TouchEvent dispatch test.

  if (!opts.skipMobile) {
    logger.writeLine("\n========== MOBILE TEST SUITE ==========");

    // -----------------------------------------------
    // MOBILE TEST 12: Viewport meta tag audit
    // Checks presence, correctness, and accessibility of <meta name="viewport">.
    logger.writeLine("[Mobile test 12/17] Viewport meta tag...");
    try {
      await bootFresh();
      ...
      const metaResult = await page.evaluate(() => {
        ...
        const tokens = content.split(",").map(t => t.trim().toLowerCase());

        const hasDeviceWidth = tokens.some(t => t.includes("width=device-width"));
        const hasInitialScale = tokens.some(t => /initial-scale\s*=\s*1(\.0)?$/.test(t));
        const hasUserScalableNo = tokens.some(t => /user-scalable\s*=\s*no/.test(t));
        const hasMaxScaleOne = tokens.some(t => /maximum-scale\s*=\s*1(\.0)?$/.test(t));

        return { found: true, content, hasDeviceWidth, hasInitialScale, hasUserScalableNo, hasMaxScaleOne };
      });

      if (!metaResult.found) {
        failures.push({ type: "MOBILE_META", where: "VIEWPORT_META", msg: '<meta name="viewport"> tag not found — mobile browsers will render at desktop zoom' });
      } else {
        if (!metaResult.hasDeviceWidth) {
          failures.push({ type: "MOBILE_META", where: "VIEWPORT_META", msg: `Viewport meta missing "width=device-width" — content: "${metaResult.content}"` });
        } else {
          notes.push({ type: "NOTE", where: "VIEWPORT_META", msg: `✓ viewport has width=device-width` });
        }
        if (!metaResult.hasInitialScale) {
          notes.push({ type: "NOTE", where: "VIEWPORT_META", msg: `Viewport meta missing "initial-scale=1" — content: "${metaResult.content}"` });
        } else {
          notes.push({ type: "NOTE", where: "VIEWPORT_META", msg: `✓ viewport has initial-scale=1` });
        }
        if (metaResult.hasUserScalableNo) {
          failures.push({ type: "MOBILE_META", where: "VIEWPORT_META", msg: `Viewport has "user-scalable=no" — this blocks accessibility pinch-zoom (WCAG 1.4.4 violation)` });
        }
        if (metaResult.hasMaxScaleOne) {
          failures.push({ type: "MOBILE_META", where: "VIEWPORT_META", msg: `Viewport has "maximum-scale=1" — this effectively blocks pinch-zoom on iOS even without user-scalable=no` });
        }
        if (!metaResult.hasUserScalableNo && !metaResult.hasMaxScaleOne) {
          notes.push({ type: "NOTE", where: "VIEWPORT_META", msg: `✓ Viewport does not block user zoom (accessibility OK)` });
        }
      }
    } catch (e) {
      notes.push({ type: "NOTE", where: "VIEWPORT_META", msg: `Viewport meta test threw: ${e && e.message ? e.message : String(e)}` });
    }

    // -----------------------------------------------
    // MOBILE TEST 13: Horizontal overflow — portrait (390x844)
    // Samples first, middle, and last sections and checks scrollWidth vs innerWidth.
    // Any section that causes horizontal scrolling is flagged as MOBILE_OVERFLOW.
    logger.writeLine("[Mobile test 13/17] Horizontal overflow — portrait...");
    try {
      // Sample first, mid, and last sections (distinct IDs)
      const sampleIdxs = [0, Math.floor(sectionsOrdered.length / 2), sectionsOrdered.length - 1];
      const sampleIds = [...new Set(sampleIdxs.map(i => sectionsOrdered[i]).filter(Boolean))];

      for (const sectionId of sampleIds) {
        await bootFresh();
        await ensureAtSection(sectionId);
        await sleep(80);
        ...
        const docOverflow = overflow.docScrollWidth > overflow.innerWidth + 1; // +1 for sub-pixel rounding
        const bodyOverflow = overflow.bodyScrollWidth > overflow.innerWidth + 1;

        if (docOverflow || bodyOverflow) {
          failures.push({
            type: "MOBILE_OVERFLOW",
            where: sectionId,
            msg: `Horizontal overflow at 390px: docScrollWidth=${overflow.docScrollWidth}px, bodyScrollWidth=${overflow.bodyScrollWidth}px, innerWidth=${overflow.innerWidth}px`,
          });
        } else {
          notes.push({
            type: "NOTE",
            where: sectionId,
            msg: `✓ No horizontal overflow at 390px (scrollWidth=${overflow.docScrollWidth}px)`,
          });
        }
      }
    } catch (e) {
      notes.push({ type: "NOTE", where: "MOBILE_OVERFLOW", msg: `Overflow test threw: ${e && e.message ? e.message : String(e)}` });
    }

    // -----------------------------------------------
    // MOBILE TEST 14: Touch target size audit
    // Navigates to a section with choices and one without, measures getBoundingClientRect()
    // on all interactive elements, and flags anything below the 44px Apple HIG minimum.
    // .page-link inline anchors are almost always under 44px and are the primary finding here.
    logger.writeLine("[Mobile test 14/17] Touch target size audit...");
    try {
      const MIN_TARGET_PX = 44;

      // Pick a section with choices and one without (for bookmark-btn)
      const choiceSectionId = sectionsOrdered.find(id => (outDegree.get(id) || []).length >= 2) || startId;
      const noChoiceSectionId = sectionsOrdered.find(id => (outDegree.get(id) || []).length === 0) || startId;
      const testSections = [...new Set([choiceSectionId, noChoiceSectionId])];
      ...
      for (const sectionId of testSections) {
        ...
        const results = await page.evaluate((minPx) => {
          const active = document.querySelector("section.game-section.active");
          if (!active) return [];

          const selectors = [
            { label: ".page-link", els: Array.from(active.querySelectorAll(".page-link")) },
            { label: ".bookmark-btn", els: Array.from(active.querySelectorAll(".bookmark-btn")) },
          ];

          // Global fixed elements — measured from document
          selectors.push({ label: ".control-btn", els: Array.from(document.querySelectorAll(".control-btn")) });
          selectors.push({ label: ".page-nav", els: Array.from(document.querySelectorAll(".page-nav:not(.disabled)")) });

          const findings = [];
          for (const { label, els } of selectors) {
            for (const el of els) {
              const rect = el.getBoundingClientRect();
              if (rect.width === 0 && rect.height === 0) continue; // hidden
              const tooNarrow = rect.width < minPx;
              const tooShort = rect.height < minPx;
        ...
        for (const r of results) {
          notes.push({
            type: "TOUCH_TARGET",
            where: sectionId,
            msg: `${r.label} "${r.text}" is ${r.w}×${r.h}px — below ${MIN_TARGET_PX}px minimum` +
              (r.tooNarrow && r.tooShort ? " (both too narrow and too short)" :
               r.tooNarrow ? " (too narrow)" : " (too short)"),
          });
        }

        if (results.length === 0) {
          notes.push({ type: "NOTE", where: sectionId, msg: `✓ All interactive elements meet ${MIN_TARGET_PX}px touch target minimum` });
        }
      }
    } catch (e) {
      notes.push({ type: "NOTE", where: "TOUCH_TARGET", msg: `Touch target test threw: ${e && e.message ? e.message : String(e)}` });
    }

    // -----------------------------------------------
    // MOBILE TEST 15: Swipe navigation
    // The book's swipe logic uses plain globals: touchStartX, touchEndX, handleSwipe().
    // Because these are NOT inside an IIFE, page.evaluate() can access them directly.
    // Tests three swipe scenarios and then dispatches real TouchEvents to verify the
    // event listeners on document also fire correctly.
    //
    // Swipe direction convention (matches the book's handleSwipe):
    //   Right-to-left (touchStartX > touchEndX, diff > 0)  → goToNextPage()
    //   Left-to-right (touchStartX < touchEndX, diff < 0)  → goToPrevPage()
    //   Sub-threshold (|diff| < 50)                        → no navigation
    logger.writeLine("[Mobile test 15/17] Swipe navigation...");
    try {
      // Pick a mid-book section that has both prev and next in DOM order
      const midIdx = Math.floor(sectionsOrdered.length / 2);
      const swipeStartId = sectionsOrdered[midIdx];
      const expectedNextId = nextMap.get(swipeStartId);
      const expectedPrevId = prevMap.get(swipeStartId);

      if (!expectedNextId || !expectedPrevId) {
        notes.push({ type: "NOTE", where: "SWIPE", msg: "Could not find a section with both prev and next neighbours; swipe test skipped." });
      } else {
        // ── (a) Right-to-left swipe → goToNextPage ──
        await bootFresh();
        await ensureAtSection(swipeStartId);
        await sleep(80);

        const swipeNextResult = await page.evaluate((expectedId) => {
          if (typeof handleSwipe !== "function") return { skip: true, reason: "handleSwipe not accessible" };
          // Right-to-left: diff = start - end = 300 - 50 = 250 → > threshold (50) → goToNextPage
          window.touchStartX = 300;
          window.touchEndX = 50;
          try { handleSwipe(); } catch (e) { return { skip: false, err: e.message }; }
          const active = document.querySelector("section.game-section.active");
          return { skip: false, activeId: active ? active.id : null };
        }, expectedNextId);

        if (swipeNextResult.skip) {
          notes.push({ type: "NOTE", where: "SWIPE", msg: `Right-to-left swipe test skipped: ${swipeNextResult.reason}` });
        } else if (swipeNextResult.err) {
          failures.push({ type: "SWIPE", where: swipeStartId, msg: `Right-to-left swipe threw: ${swipeNextResult.err}` });
        } else if (swipeNextResult.activeId !== expectedNextId) {
          failures.push({
            type: "SWIPE",
            where: swipeStartId,
            msg: `Right-to-left swipe: expected ${expectedNextId}, got ${swipeNextResult.activeId ?? "null"}`,
          });
        } else {
          notes.push({ type: "NOTE", where: swipeStartId, msg: `✓ Right-to-left swipe navigated to ${expectedNextId}` });
        }

        // ── (b) Left-to-right swipe → goToPrevPage ──
        await bootFresh();
        await ensureAtSection(swipeStartId);
        await sleep(80);

        const swipePrevResult = await page.evaluate((expectedId) => {
          if (typeof handleSwipe !== "function") return { skip: true };
          // Left-to-right: diff = start - end = 50 - 300 = -250 → < -threshold → goToPrevPage
          window.touchStartX = 50;
          window.touchEndX = 300;
          try { handleSwipe(); } catch (e) { return { skip: false, err: e.message }; }
          const active = document.querySelector("section.game-section.active");
          return { skip: false, activeId: active ? active.id : null };
        }, expectedPrevId);

        if (swipePrevResult.activeId !== expectedPrevId && !swipePrevResult.skip && !swipePrevResult.err) {
          failures.push({
            type: "SWIPE",
            where: swipeStartId,
            msg: `Left-to-right swipe: expected ${expectedPrevId}, got ${swipePrevResult.activeId ?? "null"}`,
          });
        } else if (!swipePrevResult.skip && !swipePrevResult.err) {
          notes.push({ type: "NOTE", where: swipeStartId, msg: `✓ Left-to-right swipe navigated to ${expectedPrevId}` });
        }

        // ── (c) Sub-threshold swipe (< 50px) → should NOT navigate ──
        await bootFresh();
        await ensureAtSection(swipeStartId);
        await sleep(80);

        const swipeNoopResult = await page.evaluate(() => {
          if (typeof handleSwipe !== "function") return { skip: true };
          window.touchStartX = 100;
          window.touchEndX = 70; // diff = 30, below 50px threshold
          try { handleSwipe(); } catch (e) { return { skip: false, err: e.message }; }
          const active = document.querySelector("section.game-section.active");
          return { skip: false, activeId: active ? active.id : null };
        });

        if (!swipeNoopResult.skip && !swipeNoopResult.err && swipeNoopResult.activeId !== swipeStartId) {
          failures.push({
            type: "SWIPE",
            where: swipeStartId,
            msg: `Sub-threshold swipe (30px, below 50px minimum) navigated away to ${swipeNoopResult.activeId} — should not have fired`,
          });
        } else if (!swipeNoopResult.skip && !swipeNoopResult.err) {
          notes.push({ type: "NOTE", where: swipeStartId, msg: `✓ Sub-threshold swipe (30px) correctly did not navigate` });
        }

        // ── (d) Real TouchEvent dispatch → event listener path ──
        // Creates a hasTouch browser context to ensure TouchEvent constructor is available,
        // navigates to the swipe-start section, dispatches genuine touchstart/touchend
        // events on document, and verifies the active section changes.
        try {
          ...
          // Navigate to swipe-start section via engine
          await touchPage.evaluate((n) => {
            if (typeof goToSection === "function") goToSection(n, false);
          }, numFromSectionId(swipeStartId));
          await sleep(200);

          // Dispatch touchstart then touchend for a right-to-left swipe (next page)
          await touchPage.evaluate(() => {
            function makeTouch(type, x, y) {
              try {
                const t = new Touch({ identifier: Date.now(), target: document.body, clientX: x, clientY: y, screenX: x, screenY: y, pageX: x, pageY: y });
                document.dispatchEvent(new TouchEvent(type, { touches: type === "touchend" ? [] : [t], changedTouches: [t], bubbles: true, cancelable: true }));
              } catch {}
            }
            makeTouch("touchstart", 300, 400);
            makeTouch("touchend", 50, 400);
          });
          await sleep(200);

          const touchActiveId = await touchPage.evaluate(() => {
            const a = document.querySelector("section.game-section.active");
            return a ? a.id : null;
          });

          if (touchActiveId === expectedNextId) {
            notes.push({ type: "NOTE", where: swipeStartId, msg: `✓ Real TouchEvent dispatch triggered swipe-next to ${expectedNextId}` });
          } else if (touchActiveId === swipeStartId) {
            // TouchEvent might not have fired handleSwipe() — report as note, not failure,
            // since the global-var path already proved the logic works.
            notes.push({ type: "NOTE", where: "SWIPE", msg: `TouchEvent dispatch did not trigger navigation (landed on ${touchActiveId}); swipe logic confirmed via global-var path` });
          } else {
            notes.push({ type: "NOTE", where: "SWIPE", msg: `TouchEvent dispatch landed on ${touchActiveId ?? "null"} (expected ${expectedNextId})` });
          }

          await touchContext.close();
        } catch (e) {
          notes.push({ type: "NOTE", where: "SWIPE", msg: `Real TouchEvent test threw: ${e && e.message ? e.message : String(e)}` });
        }
      }
    } catch (e) {
      notes.push({ type: "NOTE", where: "SWIPE", msg: `Swipe test threw: ${e && e.message ? e.message : String(e)}` });
    }

    // -----------------------------------------------
    // MOBILE TEST 16: Landscape layout (844x390)
    // Switches viewport to landscape, navigates to the first game section, and checks
    // that there is no horizontal overflow and both page-nav arrows are within viewport bounds.
    // Restores to portrait (390x844) at the end.
    logger.writeLine("[Mobile test 16/17] Landscape layout...");
    try {
      ...
      const landscapeResult = await page.evaluate(() => {
        ...
        return {
          ...
          vw,
          vh,
          navLeft: rectInfo(navLeft),
          navRight: rectInfo(navRight),
        };
      });

      // Horizontal overflow
      if (landscapeResult.overflowW > landscapeResult.vw + 1) {
        failures.push({
          type: "MOBILE_OVERFLOW",
          where: "LANDSCAPE",
          msg: `Horizontal overflow in landscape (844×390): scrollWidth=${landscapeResult.overflowW}px, innerWidth=${landscapeResult.vw}px`,
        });
      } else {
        notes.push({ type: "NOTE", where: "LANDSCAPE", msg: `✓ No horizontal overflow in landscape (scrollWidth=${landscapeResult.overflowW}px at ${landscapeResult.vw}px viewport)` });
      }

      // Nav arrow bounds
      for (const [arrowName, rectData] of [["page-nav-left", landscapeResult.navLeft], ["page-nav-right", landscapeResult.navRight]]) {
        if (!rectData) {
          notes.push({ type: "NOTE", where: "LANDSCAPE", msg: `${arrowName} not found in landscape` });
          continue;
        }
        const outOfBounds = rectData.left < 0 || rectData.right > landscapeResult.vw || rectData.top < 0 || rectData.bottom > landscapeResult.vh;
        if (outOfBounds) {
          failures.push({
            type: "MOBILE_LAYOUT",
            where: "LANDSCAPE",
            msg: `${arrowName} is out of viewport bounds in landscape: rect=${JSON.stringify(rectData)}, viewport=${landscapeResult.vw}×${landscapeResult.vh}`,
          });
        } else {
          notes.push({ type: "NOTE", where: "LANDSCAPE", msg: `✓ ${arrowName} within bounds in landscape (${rectData.left}–${rectData.right}px, ${rectData.w}×${rectData.h})` });
        }
      }
      ...
      notes.push({ type: "NOTE", where: "LANDSCAPE", msg: `Landscape test threw: ${e && e.message ? e.message : String(e)}` });
      try { await page.setViewportSize({ width: 390, height: 844 }); } catch {}
    }

    // -----------------------------------------------
    // MOBILE TEST 17: iOS input zoom prevention
    // iOS Safari auto-zooms the page when a focused <input> has font-size < 16px.
    // This is jarring in a book UX and disorienting for the reader.
    // Checks the computed font-size on the #page-jump input field.
    logger.writeLine("[Mobile test 17/17] iOS input zoom prevention...");
    try {
      await bootFresh();
      await ensureAtSection(startId);

      const inputResult = await page.evaluate(() => {
        const INPUT_SELECTORS = ["#page-jump", 'input[type="number"]', 'input[type="text"]', 'input[type="range"]'];
        const findings = [];
        for (const sel of INPUT_SELECTORS) {
          ...
          findings.push({ sel, computedFontSize: fs, raw: computed.fontSize, tooSmall: fs < 16 });
        }
        return findings;
      });

      for (const r of inputResult) {
        if (r.tooSmall) {
          notes.push({
            type: "MOBILE_INPUT_ZOOM",
            where: "INPUT_FONT_SIZE",
            msg: `"${r.sel}" has computed font-size ${r.computedFontSize}px (raw: "${r.raw}") — iOS Safari will auto-zoom on focus if font-size < 16px`,
          });
        } else {
          notes.push({ type: "NOTE", where: "INPUT_FONT_SIZE", msg: `✓ "${r.sel}" font-size is ${r.computedFontSize}px (>= 16px, safe from iOS auto-zoom)` });
        }
      }

      if (inputResult.length === 0) {
        notes.push({ type: "NOTE", where: "INPUT_FONT_SIZE", msg: "No <input> elements found to test for iOS zoom." });
      }
    } catch (e) {
      notes.push({ type: "NOTE", where: "INPUT_FONT_SIZE", msg: `iOS zoom test threw: ${e && e.message ? e.message : String(e)}` });
    }

    logger.writeLine("========== END MOBILE TEST SUITE ==========");

  } else {
    notes.push({ type: "NOTE", where: "MOBILE", msg: "Mobile test suite skipped (--skipMobile)." });
  }
