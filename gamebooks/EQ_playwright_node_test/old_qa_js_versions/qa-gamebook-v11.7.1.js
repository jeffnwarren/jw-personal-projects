...
// Copy the gamebook html (e.g. Dungeon_of_Dread_Final_2026-02-20_0727_CST.html) and latest qa_gamebook-<version>.js file (e.g. qa-gamebook-v11.7.1.js) to the project folder.
//
// PURPOSE:
...
//
// NEW IN v11.7:
// Controlled bottom-bookmark CLICK TEST (once per visited section, max):
//   If the bottom "Back to page X" bookmark is visible, we click it ONE time for that section
//   to prove it actually navigates to page X. This is a verification-only check:
//   it does NOT enqueue new BFS states, so it won’t blow up the traversal.
//
...
//
// USAGE:
//   node qa-gamebook-v11.7.js [flags] "C:\path\to\YourGamebook.html"
//
// Example:
//   node qa-gamebook-v11.7.js --headed --progress --slowMo=50 --outputLog "C:\path\to\YourGamebook.html"
//
// NOTES:
...
// -----------------------------------------
// CLI options
//   node qa-gamebook-v11.7.js [--headed] [--progress] [--progressEvery=25] [--maxStates=5000] [--slowMo=0] [--timeoutMs=1500] "path/to/book.html"
...
function makeLogger(outputLogOpt, bookFilePath) {
  ...
  // Write using appendFileSync so the file grows immediately (tail-friendly).
  function appendLine(s) {
    try { fs.appendFileSync(fullPath, s, { encoding: "utf8" }); } catch {}
  }

  // Tee console + stdout/stderr
  const orig = {
    log: console.log,
    error: console.error,
    warn: console.warn,
  ...
  console.log = (...args) => {
    orig.log(...args);
    appendLine(normalizeArgs(args) + "\n");
  };
  console.error = (...args) => {
    orig.error(...args);
    appendLine("ERROR: " + normalizeArgs(args) + "\n");
  };
  console.warn = (...args) => {
    orig.warn(...args);
    appendLine("WARN: " + normalizeArgs(args) + "\n");
  };
  ...
  // Capture progress dots and any raw writes
...
(async () => {
  ...
  if (!filePath) {
    console.error('Usage: node qa-gamebook-v11.7.js [--headed] [--progress] [--progressEvery=25] [--maxStates=5000] [--slowMo=0] [--timeoutMs=1500] [--outputLog[=<name>]] "C:\\path\\to\\Dungeon_of_Dread.html"');
    process.exit(1);
  }
...
for (let i = 0; i < sectionsOrdered.length; i++) {
  ...
  while (queue.length && states < maxStates) {
    ...
    // Controlled bottom-bookmark CLICK TEST (verification-only; does NOT affect BFS)
    // If the bottom bookmark is visible, click it once for this section and verify it lands on the expected page.
    if (state.lastChoice && ui.bottom && ui.bottom.visible && !testedBottomBookmark.has(state.at)) {
      ...
      // Only run the click test if we can parse a target number.
      if (Number.isFinite(targetNum)) {
        testedBottomBookmark.add(state.at);
        const expectedId = `section-${targetNum}`;
        try {
          // Click the bookmark button inside the active section
          ...
          // Return to the original section so traversal remains unchanged.
  ...
  // IMPORTANT: Don't call logger.close() until AFTER we've printed the final summary.
  // logger.close() restores console/stdout hooks back to original, which would prevent the
  // summary lines from being captured into the output log.
